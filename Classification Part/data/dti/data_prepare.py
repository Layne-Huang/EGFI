...
def data_split(full_list, ratio, shuffle=False):
    ...
    sublist_1 = full_list[:offset]  #1:[3060:] [:3036] 2:[3060:6056] 3:[6056:9077] 4:[9077:12098] 5:[12098:15119]
    sublist_2 = full_list[offset:]
    print(len(sublist_1))
    return sublist_1, sublist_2

def read_data(file_dir, filename, split=False):
    data = []
    ...
    random.shuffle(data)
    if split:
        train_data, test_data = data_split(data, ratio=0.9,shuffle=True)
...
def get_dataset(file_dir, filename, max_length, split = False):
    if split:
        train_data, test_data = read_data(file_dir, filename, split=split)
...
def dump_dataset(data_name, split=False):
    if split:
        file_path = 'large_testdata\\'
        train_dataset, test_dataset = get_dataset(file_dir='.', filename=data_name + '.json',
                                                  max_length=config.MAX_LENGTH, split=split)
        torch.save(train_dataset, file_path + 'train_90%' + '.pt')
        torch.save(test_dataset, file_path + 'test_10%' + '.pt')
    else:
