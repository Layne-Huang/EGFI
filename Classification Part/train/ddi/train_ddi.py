...
def train(config, log_path, lr, k):
    ...
    writer = SummaryWriter('../../pic/drugmask_normal'+str(lr))
    ...
    # 加载dataloader
    full_dataset = torch.load(os.path.join(config.ROOT_DIR, 'train_c.pt'))
    train_dataset, valid_dataset = random_split(full_dataset, [len(full_dataset)-int(len(full_dataset)*0.2352), int(len(full_dataset)*0.2352)])
    train_loader = DataLoader(train_dataset, config.BATCH_SIZE, shuffle=True)
    valid_loader = DataLoader(train_dataset, config.BATCH_SIZE, shuffle=False)
        ...
        for batch_iter, train_batch in enumerate(train_loader):
            ...
            if (batch_iter + 1) % validate_every == 0:
                ...
                if valid_f1 > best_f1:
                    ...
                    if best_f1<0.90 or test_f1<0.83:
                        print(r'Get the new best model of valid F1: {}, test F1:{}, but do not save it because F1 is less than 0.90'.format(best_f1, test_f1))
                    else:
                        torch.save(model.state_dict(), model_path)
                        print(r'Saved the new best model (valid F1: {}; test F1: {}) to {}'.format(best_f1, test_f1, model_path))
...
if __name__ == '__main__':
    ...
    log_t = open('../../log/drugmask_normal_time.log', 'a')
    for lr in range(1, 4):
        # if lr==3:
        #     continue
        start = time.time()
        ...
        config.log()
        F = train(config, '../../log/drugmask_normal_e-5.log', lr, 10)
        end = time.time()
